...
(defmethod encounter-process ((encounter merchant))
  (show-player)
  (princ #\newline)
  (custom-print `(the merchant resources for ,(merchant-exchange-rate encounter) money))
  (custom-print `(you can sell resources for ,(- (merchant-exchange-rate encounter) 1) money))
  (custom-print `(the merchant is selling repairs for ,(merchant-repair-cost encounter) money))
  (princ #\newline)
  (gain-rewards '(sell buy repair leave)
	        `((sell (progn (if (> ,*resources* 0)
			   	   (progn (decf *resources*)
				          (gain-money ,(- (merchant-exchange-rate encounter) 1)))
				   (custom-print '(you do not have any resources)))
			       (encounter-process ,encounter)))
	          (buy (progn (if (<= ,(merchant-exchange-rate encounter) ,*money*)
		       	      	  (progn (decf *money* ,(merchant-exchange-rate encounter))
				  	 (incf *resources*))
				  (custom-print '(you do not have enough money)))
			      (encounter-process ,encounter)))
		  (repair (progn (if (<= ,(merchant-repair-cost encounter) ,*money*)
		  	  	     (progn (decf *money* ,(merchant-repair-cost encounter))
				     	    (incf *player-health*))
				     (custom-print '(you do not have enough money)))
				 (encounter-process ,encounter)))
		  (leave (progn ,(encounter-on-finish encounter)
		       	 	(custom-print '(you have left the merchant)))))))
